...
def load_multihot_problem_to_skill(args):
    ...
    print("问题数",max_problem_id+1)
    # 找出最大技能数
    max_skill_id = max(list(set(skill_ids)))
    print("技能数",max_skill_id+1)
    
    # 获得问题多技能multi-hot embedding 最后一位代表0向量
    ...
    for key in dict:
        ...
        for item in skills:
            arr[int(item)] = 1
        emb[int(key)] = arr
    # 最后一位代表0向量 用于shapley value mask的基准值
    emb[-1] = [0 for i in range(max_skill_id+1)]

    # print('编号为40的题目对应的技能one-hot编码')
    # print(emb[40])
...
def load_data(args):
    ...
    for train_or_test in ['train', 'test']:
        ...
        dataList_dict[train_or_test] = file_to_list(filePath_dict[train_or_test], args.min_seq_len, args.max_seq_len,args.data_set)
